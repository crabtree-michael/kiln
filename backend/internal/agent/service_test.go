...
func TestSend_DoesNotBlockOnTheProvider(t *testing.T) {
	...
	svc := agent.NewService(newFakeStore(), provider, &fakeEvents{}, &fakeSlots{}, testutil.NewFakeClock(), nil)
...
func TestSend_RecordsTheDecodedPayload(t *testing.T) {
	store := newFakeStore()
	svc := agent.NewService(store, newBlockingProvider(), &fakeEvents{}, &fakeSlots{}, testutil.NewFakeClock(), nil)
