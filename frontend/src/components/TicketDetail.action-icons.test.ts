// Accept and Delete are supposed to be the MICROPHONE, wearing different glyphs:
// same disc, same fill, same outline, same raised shadow that collapses under the
// thumb. Nothing else in the gate can see that. jsdom does no layout, so the DOM
// tests next door only know the two buttons exist; and the mic is dressed in a
...
describe('TicketDetail Accept/Delete wear the mic’s treatment', () => {
  ...
  it('is not re-skinned by the primary surface', () => {
    // The old skin gave Accept an accent pill and Delete a 40px danger circle at
    // higher specificity, which is how the footer would quietly drift back apart
    // on the one surface the app actually renders.
  ...
  it('answers a press, never a hover — the mic doesn’t either', () => {
    ...
    // one of these would paint it as pressed.
