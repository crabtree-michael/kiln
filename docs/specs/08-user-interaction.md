...
- **Header** — the Kiln mark and a one-line status summary derived from the feed:
  `1 blocker · 4 updates` when something needs the user, `5 streams · nothing needs
  you` when not, `all clear` when the feed is empty. (A *stream* is the design's word
...
- **`notifications` table** (runtime module, beside `messages` — `07` §3): `id
  bigserial`, `kind` (`update`/`preview`, CHECK), `ticket_id` nullable, `body text`,
