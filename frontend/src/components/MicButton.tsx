...
// it in.
//
// It is the orb and nothing else, on every surface: each placement renders its own
// send/discard around it (the dock's controls row, the desktop composer, the
// ticket sheet's `TicketDetailVoiceActions`). It used to carry an opt-in
// `sendable` mode in which the orb GAVE WAY to send + clear once a transcript
// landed — that is gone, because the sheet now keeps the orb on screen while
// speaking (it is the expression indicator: the glow is the only thing reporting
// that the mic is live) and puts its two actions beside it instead.
//
...
export function MicButton({ ticketContext }: MicButtonProps): JSX.Element {
  const { micState, connecting, pause, resume, getLevel, setTicketContext } = useVoice();
