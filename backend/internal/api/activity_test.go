...
func TestActivityStatusReflectsThinkingBracket(t *testing.T) {
	...
	// GET /api/activity is withProject-gated (11 phase 2), so the test server
	// needs the session+project harness the other app-route tests use.
	...
	if err := hub.PushActivity(context.Background(), testProjectID, thinkingEvent(true)); err != nil {
	...
	if err := hub.PushActivity(context.Background(), testProjectID,
	...
	if err := hub.PushActivity(context.Background(), testProjectID, thinkingEvent(false)); err != nil {
