...
// port so Store stays scoped to the two queue tables (04 §2).
type MessageStore interface {
	...
	AppendKilnMessage(ctx context.Context, text string) (Message, error)

	// Recent returns the last n transcript rows, oldest first (07 §3).
	// Backs both the brain's ConversationReader (06 §3.2, adapted at the
	// composition root) and GET /api/messages (07 §4).
...
// ({message_id, text, at}) to every connected client after the kiln row is
// durably appended. Implemented by the api package's Hub.
type SayPusher interface {
	PushSay(ctx context.Context, m Message) error
}
