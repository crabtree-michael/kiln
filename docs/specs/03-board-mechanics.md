...
**Date:** 2026-07-03
**Status:** Proposed — revised same-day: provider-neutral rename (Sandbox → Worker,
`agent.send`/`agent.release` topics) per `05`
...
- Each entity (Ticket, Worker, outbox entry) — fields, valid states, invariants (§2–§3).
...
- How the WIP cap (= available workers) is enforced atomically (§3, §5).
...
agent-runtime payloads (`05`); the client-facing snapshot shape (wire schema,
...
| `state`   | Column (derived) | Zone (derived) | Worker bound? |
| --------- | ---------------- | -------------- | -------------- |
...
| `worker_id`                | uuid, null        | FK → `workers`. Non-null iff `working`/`blocked`.                               |
...
A worker row is a **capacity slot**, not a live resource handle — `01` §5's "sandbox" is
realized as this provider-neutral slot. The table is seeded with N
...
The board knows nothing about the provider behind a worker — no refs, no sessions, no
workers. The agent-runtime module (`05`) owns the worker↔provider mapping in its own
state, joined by this row's `id`.


A worker has no `status` column. **Free vs busy is derived**: a worker is busy iff an active
...
| I2  | **WIP cap:** at most one active (`working`/`blocked`) ticket per worker; Developing therefore holds ≤ N tickets. | Partial unique index on `tickets(worker_id) WHERE state IN ('working','blocked')`; N = row count of `workers`.                           |
| I3  | `worker_id` is non-null iff `state` is `working`/`blocked`.                                                      | CHECK constraint.                                                                                                                           |
...
| `SendToAgent`          | id, instruction              | state ∈ {`working`, `blocked`} | → `working`, clears `blocked_reason`             | `agent.send`                       |
| `MarkBlocked`          | id, reason                   | state = `working`              | → `blocked`, sets `blocked_reason`               | `notify.send`                      |
| `AcceptToDone`         | id                           | state ∈ {`working`, `blocked`} | → `done`, clears `worker_id` + `blocked_reason`  | `pull.evaluate`, `agent.release`   |
| `GetBoard`             | —                            | —                              | none (read)                                      | —                                  |
| `RunPull` *(internal)* | —                            | see §5                         | `ready` → `working` per pulled ticket            | `agent.send` per pulled ticket |


Notes:

- `SendToAgent` **covers two** `01` **§5 rows** — Blocked→Working (resume with the user's answer)
and Working→Working (new turn). Same mechanics: persist the intent, emit `agent.send`.
...
total/free worker counts. Its wire shape lives in `/schema`.
...
exists AND a worker is free.

**Trigger.** Exactly two operations can make the pull condition become true: `MarkReady`
(creates a ready ticket) and `AcceptToDone` (frees a worker). Both emit a `pull.evaluate`
...
BEGIN;
  ...
  -- a free worker = no active ticket references it
  ...
  UPDATE tickets SET state = 'working', worker_id = $worker WHERE id = $ticket;
...
- `FOR UPDATE` on the worker row serializes claimers; `SKIP LOCKED` makes the loser pick a
*different* free worker (or stop) instead of blocking — the claim is atomic and
contention-free.
...
| ---------------- | --------------------------- | ---------------------------------- | ----------------------------------------------------------- |
| `agent.send` | `RunPull` (title + body as the work instruction), `SendToAgent` (the instruction) | agent-runtime adapter (`05`) | ticket id, worker id, message |
| `agent.release` | `AcceptToDone` | agent-runtime adapter (`05`) | worker id |
...
are made safe to repeat: the entry `id` travels as the idempotency key on `agent.send` /
`agent.release` (the agent-runtime module — and its mock — must honor it: `05` §7), duplicate `pull.evaluate` and
...
- `agent.send` failed → the agent never got the message: the runtime
...
Changing capacity = inserting a worker row (takes effect on next pull) or deleting a free
...
- **Postgres adapter** — implements the store port; owns the §8 DDL/migrations and the row-locking queries. Injected at the c omposition root (`02` §7).
- **No agent-runtime port.** The outbox decision (§7) means the board never calls the agent
platform — it appends intent rows; the runtime's drain loop invokes the agent-runtime
module (`05`). This supersedes `02` §5's topology sketch of an Amika port inside board
(§10, D5). The board's only infrastructure dependency is Postgres.
...
rows *is* asserting side effects, no agent-runtime fake needed. Integration tests run against real
...
| D2  | WIP cap is structural: N worker rows + partial unique index; free/busy derived.  | `status` column on workers; a WIP counter.                                           | Nothing to keep in sync, so nothing can drift; atomicity comes from row locking + the index, not from disciplined bookkeeping.                                                                                          |
...
(runtime spec, `02` §7); worker↔provider lifecycle and message payload details (agent-runtime
spec, `05`); whether the shaping
