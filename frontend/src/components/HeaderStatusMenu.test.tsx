...
describe('HeaderStatusMenu', () => {
  ...
  it('opens straight onto the tickets, with no head naming the list', () => {
    // The panel opens from a button that already reports what it holds, so a
    // "Tickets" line above the rows named the list twice. The rows are the
    // panel's first content now.
    render(<HeaderStatusMenu summary={summary} board={board} />);
    fireEvent.click(screen.getByRole('button'));

    expect(screen.queryByText('Tickets')).toBeNull();
    expect(document.querySelector('[data-role="header-status-heading"]')).toBeNull();
    const panel = document.querySelector('[data-role="header-status-panel"]');
    expect(panel?.firstElementChild).toHaveAttribute('data-role', 'header-status-list');
  });
