...
func TestSend_DoesNotBlockOnTheProvider(t *testing.T) {
	...
	svc := newService(newFakeStore(), provider, &fakeEvents{}, &fakeSlots{}, testutil.NewFakeClock(), nil)
...
func TestSend_RecordsTheDecodedPayload(t *testing.T) {
	store := newFakeStore()
	svc := newService(store, newBlockingProvider(), &fakeEvents{}, &fakeSlots{}, testutil.NewFakeClock(), nil)
