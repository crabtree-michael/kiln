...
- [ ] Exactly one instance runs the four background loops at any time, across a rolling deploy.
- [ ] The non-leader instance still serves HTTP and SSE normally — no user-visible degradation
      during the overlap window.
- [ ] The non-leader retries and takes over within a few seconds of the leader exiting, on both
      clean shutdown and hard kill.
- [ ] Leadership acquisition/loss is logged, with the process-boot instance id (§6 item 6) so the
      transition is legible in Render's logs.
- [ ] A leader that loses its connection stops the loops rather than continuing to run un-locked.
