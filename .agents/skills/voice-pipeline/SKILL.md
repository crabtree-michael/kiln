...
- **Layout gate — the speaking arrangement is reachable offline: `speak(page, mic, text)` in
  `tests/layout/harness.ts`.** jsdom performs no layout, so where the dock's send/× actually
  LAND needs a browser, and the mic is what stood between the gate and that state. `mountShell`
  installs the stand-ins and the real store runs its real machine behind them (no key, no
  network, no billing): Chromium's fake capture device (config `launchOptions` +
  `permissions`), a stubbed `/api/voice/token`, and `page.routeWebSocket` over the AssemblyAI
  host, down which `speak` pushes one **partial** `Turn` — partial on purpose, since a
  formatted final arms the auto-send and takes the controls off screen mid-spec. Three traps,
  all surfacing as the same useless `AbortError: Unable to load a worklet's module`:
  - **The token's `expires_at` must be minutes out, not years.** The refresh is scheduled as
    `setTimeout(expiry - now - buffer)`, and a delay past the 32-bit ms ceiling (~24.8 days)
    wraps to ~immediately — tearing the stream down mid-setup, with the AudioContext closing
    while `addModule` is still fetching.
  - **Vite's DEV server cannot serve the PCM worklet as a worklet module** (ES module + HMR
    preamble; a worklet scope has no imports). A prod build inlines it, so this is the gate's
    problem alone — `addModule` is patched to a same-named no-op processor.
  - **That stand-in must arrive as a `data:` URL.** A worklet fetches in its own realm, so
    `page.route` never sees the request, and a page-minted blob URL races its own registration.
...
- **Send, × and the mic are one size (54px), and on the dock they sit on the row's two FLANKS.**
  The pair used to be 40px circles hugging the mic, and both halves of that were wrong. Size:
  they are the mic's peers on every surface — the sheet's footer already dresses its glyphs at
  the mic's disc — so the two smallest targets on the row were the ones carrying the
  irreversible actions. Placement: at 54px, clustered puts "send this" and "throw this away" a
  few pixels apart under one thumb, so the 1fr/auto/1fr grid pushes send to `justify-self:
  start` in column 1 and × to `justify-self: end` in column 3, mic still centred. **Keyboard
  mode is the deliberate exception** and holds send at 40px: no orb in that row to match, and
  its peers there are the voice toggle and the dismiss button. Measured in
  `tests/layout/voice-controls.spec.ts` and `ticket-detail.spec.ts` — every one of these
  renders identically in the DOM whether it holds or not.
