...
type Store interface {
	// InsertEvent appends to the events queue (04 §6 EnqueueEvent). The
	// outbox is never written here — the board appends it transactionally
	// (03 §7).
	InsertEvent(ctx context.Context, t EventType, payload []byte) (int64, error)
	...
	// transaction that only increments attempts (04 §3 step 1). ok is false
	// when nothing is due.
	ClaimNextDue(ctx context.Context, q QueueName) (e Entry, ok bool, err error)
