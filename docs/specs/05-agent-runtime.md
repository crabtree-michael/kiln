...
**Workers** — N opaque handles (the board's capacity slots, `03` §2.3). Not sandboxes, not sessions, not jobs.

**Send** — deliver a message to a worker (a ticket's work order, an instruction, an

answer to a blocker).

**Output** — a worker's turn result arriving as an `agent.turn_completed` event.
