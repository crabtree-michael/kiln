...
type FeedCard struct {
	...
	GitHubURL   *string
	GitHubLabel *string
	// WorkSummary is set on kind=done cards (08 §7): the landed work's full
	// description (the commit message, or the PR title + description), rendered as
	// the card's expandable body — previewed and opened on tap. Nil otherwise, and
	...
	// board-derived blocker/proposal cards. The client collapses already-seen
	// cards out of the default feed with no delay, reachable again behind
	// "Show earlier"; an unseen card (nil) is never collapsed, at any age.
	SeenAt *time.Time
}
