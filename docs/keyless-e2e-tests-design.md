...
**Later addition — a sixth boundary: GitHub (`KILN_GITHUB_MODE=mock`).** The settings repo picker replaced the free-text repo URL field, so the project's repo is now chosen from the caller's connected GitHub account — which makes GitHub a *data* dependency of the onboarding form, not just an auth one. `internal/identity/githubmock` is the offline stand-in (canned repo listing), selected by `KILN_GITHUB_MODE=mock` in `newGitHub`. It also configures the synthetic credential `DevSignIn` stores (`identity.SetDevGitHubToken`), because a dev-minted session otherwise has no GitHub credential and its picker would show "Connect GitHub account" — an OAuth round trip no headless spec can complete. That opt-in is mock-mode only, so a real-service run's stored token is never overwritten. Same principle as the rest of this design: fake only the vendor — the credential store, the `GET /api/github/repos` endpoint, and the real dashboard form all run for real.
