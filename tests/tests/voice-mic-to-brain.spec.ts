...
// REAL frontend pipeline with no mocks: the mic is on by default (09 §3 D3), so
// the VoiceProvider fetches a token, opens the real AssemblyAI socket, the
...
test('speaking "This is a test" into the mic lands a human.message and the brain runs a turn', async ({
  ...
  // Open the app. Mic is on by default (09 §3 D3): mounting starts the whole
  // pipeline against the fake mic — no click, no affordance to trigger.
  await page.goto('/');
  // Sanity: the dock rendered and the app entered Listening (amber, mic on).
    ...
    .poll(
      async () => {
        userRow = (await messages(request)).find(
          (m) => m.role === 'user' && m.message_id > baselineMaxId && /test/i.test(m.text),
        );
        return userRow !== undefined;
      },
      { message: 'the spoken utterance never landed as a human.message (STT → /api/message)' },
    )
    .toBe(true);
  // eslint-disable-next-line no-console
  console.log(`voice utterance committed as user message: ${JSON.stringify(userRow?.text)}`);
