...
const testInstallationID = int64(90210)

func TestAuthConnectRedirectsToTheInstallFlow(t *testing.T) {
	// That the flow lands on the App's install page — GitHub's repository
	// chooser — is the identity service's contract (ConnectURL) and the
	// adapter's (InstallURL); what this asserts is that the route drives that
	// port and mints CSRF state doing it.
	auth := &fakeAuth{connectURL: "https://github.com/apps/kiln/installations/new"}
...
// A flow that authorized the user but installed nothing yields no repository
// access, yet the account did authenticate — so the callback signs them in and
// explains, rather than bouncing them back out. Refusing the session would lock
// them out of the retry the page offers.
func TestAuthCallbackWithoutInstallationStillSignsIn(t *testing.T) {
	auth := &fakeAuth{
		completeConnectUser: identity.User{ID: testUserID},
	...
	if sess == nil || sess.Value != testSessionToken {
		t.Fatalf("kiln_session cookie = %+v, want one minted despite the missing install", sess)
	}
}
